...
def build_bigru_model(input_shape):
    ...
    """
    Xây dựng một mạng Bidirectional GRU cơ bản
    """
...
def main():
    ...
    print("\n5. Compile và Train BiGRU...")
    # shape cho input là (time_steps, features) cụ thể là (12, 6)
    model = build_bigru_model((X_train.shape[1], X_train.shape[2]))
    model.summary()
    ...
    plt.title('Độ mất mát (Loss) của Model BiGRU qua các Epochs')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.legend()
    plt.show()
    ...
    plt.plot(predictions_denorm[:500], label='Dự đoán (Predictions) - BiGRU', color='red', alpha=0.7)
    plt.title('Dự đoán lưu lượng giao thông BiGRU vs Thực tế')
